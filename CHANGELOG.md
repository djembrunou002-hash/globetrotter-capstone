...
### Known limitations
- `.map-marker` sets `transform: rotate(-45deg)` to turn the circle into a downward-pointing teardrop, but MapLibre writes `transform` inline on the marker element every frame to position it, and inline styles win. The rotation has therefore never applied: pins are not teardrops (the square corner sits at bottom-left instead of pointing down), and the `.map-marker__badge` counter-rotation of `+45deg` *does* apply, so itinerary position numbers are tilted 45°. Fixing it means moving the shape onto a child element so MapLibre keeps the outer `transform` to itself — which would also tighten popup-to-pin alignment, since the tip would finally be where the `bottom` anchor thinks it is
...
### Added
- Contact feature on the destination detail page, giving a way to reach out about a listed destination directly from `Destinationdetails.jsx`
- Edit/write support on destination forms for both the submitting user and admins, so a destination's details can be revised from either side of the review flow rather than only at submission time

### Changed
- `user-service/config.py` — `JWTManager` now sets `JWT_ACCESS_TOKEN_EXPIRES` explicitly (`timedelta(days=...)`, defaulting to 30 days, overridable via `JWT_ACCESS_TOKEN_EXPIRES_DAYS`). Flask-JWT-Extended's unset default is 15 minutes, which is why sessions were expiring within minutes of logging in even though the token itself was sitting untouched in `localStorage`
- `Login.jsx` — removed a leftover, non-`replace` `navigate('/profile', {replace:true})` immediately followed by a second push to `/home`/`/admin`; both the password and Google sign-in paths now do a single `navigate(destination, {replace:true})`
- `Register.jsx`, `VerifyOtp.jsx`, `SelectStyle.jsx` — every post-auth `navigate()` call (email/phone sign-up, Google sign-up, OTP verification, style selection, and the "skip" action) now passes `{replace:true}`, and the unauthenticated guard redirects to `/login` the same way
- `Register.test.jsx`, `Login.test.jsx` — updated to assert `navigate(path, {replace:true})` instead of the bare path
...
### Added
- `components/PlanetLoader.jsx` + `PlanetLoader.css` — a shared loading indicator: a rotating globe drawn as inline SVG, with hand-traced continent paths (Greenland, the Americas, Europe, Africa, Asia, Oceania) clipped to the sphere, an orbit ring, and an optional caption. Two sizes, `default` and `small`. Carries `role="status"` and `aria-live="polite"`, so a screen reader announces the caption when it appears rather than the change passing silently
- Wired into every place that previously showed a bare text status or nothing at all — 14 call sites: `Landing`, `Home`, `Destinations`, `Destinationdetails`, `Itineraries`, `itineraryDetails`, `MapPage`, `Favorites`, `MyDestinations`, `MyDestinationDetails`, `DestinationForm`, `AdminDashboard`, `CommentSection`, `AddToItineraryButton`. On the map this replaces the plain status paragraph while loading, which is the gap that made a slow Geoapify route look like a broken feature

### Fixed
- `utils/LandingCache.js` renamed to `landingCache.js`. Every import already used the lowercase form, so this was a Linux build failure waiting to happen — the same class of bug as the `Confirmdialog`/`Preferencesmodal` mismatch fixed on 02-08, and the third occurrence in this project

## [15-08-2026]

### Added
- Itinerary editing. `PUT /itineraries/<id>` in itinerary-service — a partial update, so only the keys present in the body change. Changing the destination set recomputes `tags` from the new destinations (matching `create_itinerary`) and prunes `destination_times` entries for stops no longer in the trip. Ownership is enforced the same way as every other route on that blueprint: the `next()` lookup filters on `user_id`, so somebody else's itinerary is a 404 rather than a 403
  - `updateItinerary()` in `itineraryService.js`
  - `ItineraryDraftProvider` gains `editingId`, `isEditing` and `openFormForEdit(itinerary)`, which pre-fills the draft from an existing record. `openForm()` clears `editingId`, so creating after editing doesn't inherit the previous target
  - `Additinerarymodal.jsx` switches its heading and submit label on `isEditing` rather than a second component existing. Editing therefore reuses the destination picker, pre-checked with the current selection, so stops can be added or removed through the same flow
  - The itinerary card's three-dot menu is now Edit · Share · Delete
  - `itineraryCard.edit`, `addItinerary.editHeading`, `addItinerary.save`, `addItinerary.saving` in both languages
- `components/ScrollRow.jsx` — a horizontally scrollable row that shows an arrow at whichever edge has content beyond it, with a fade mask underneath. The arrows are also buttons, scrolling 70% of the visible width. A `ResizeObserver` watches the row and its children so the arrows stay correct when the tag list changes after a filter is applied, or when switching language changes pill widths. On touch devices the arrow nudges three times on first render, since there is no hover and no scrollbar to reveal that the row scrolls. Applied to all five filter rows (three on Destinations, two on Home)
  - The arrows are `aria-hidden` with `tabIndex={-1}`: keyboard and screen-reader users already reach every pill by tabbing, which scrolls the row natively, so exposing decorative duplicates would only add noise
- `utils/filterIcons.js` — an icon per filter value, rendered before the label. Covers all 13 destination types, all 76 tags and the 3 budget levels, with fallbacks (`📍`, `🏷️`, `💰`) so a tag added later through the submission flow still renders. Emoji rather than inline SVG: 92 icons as SVG would be a significant addition to a bundle already dominated by MapLibre. Keyed off the raw data value, never the translated label, so the icon is identical in every language
- Motorbike travel mode on the map. Geoapify already supports `motorcycle` and `/places/route` passes `mode` straight through, so no backend change was needed. Uses the `balanced` route profile like `drive` (shortest-path produces awkward turns on a road network) and a `FALLBACK_SPEED_MPS` of 7.5 — slightly above driving, since a bendskin filters through traffic. `map.byMotorbike` in both languages
- `components/ErrorBoundary.jsx` — wraps the `Routes` tree in `App.jsx`, keyed on `location.pathname`. React unmounts the entire tree on an uncaught render error, which is what turned a single undefined prop into a blank app that stayed blank through a back navigation; the boundary contains it to a fallback card with Go back and Reload, leaving the language, notifications and itinerary-draft providers running. The `resetKey` matters: a boundary that has caught an error stays in the error state until something resets it, so without it navigating away would still show the fallback. Its strings are deliberately hardcoded English rather than `t()` calls, since a broken render could be the translation context itself

### Fixed
- **`ShareItineraryModal.jsx` contained a duplicate of `ReorderItineraryModal`.** The file had been overwritten in commit `10a11fa` (the French translation pass) and the wrong component was translated along with everything else, so the mistake was invisible in review. `<ShareItineraryModal itinerary={...} onClose={...} />` therefore rendered the reorder component without its required `destinations` prop, hit `destinations.map(...)` on `undefined`, and took down the whole app — which also explains the "everything is empty when I go back" half of the report: that was the same crash, not a second bug. Restored from `2760d12` and converted to `t()` calls. Nearly every key it needed already existed in `translations.js` (the complete `share` block in both languages, plus `validation.invalidEmail`, `common.close`, `common.remove`, `common.loading`) — written for a component that had been clobbered before it could use them. Only `share.aria` was missing
  - The restored version also gains an `active` flag with cleanup in its `useEffect`, so closing the modal before `getSharedUsers` resolves no longer calls `setState` on an unmounted component, and `response.shared_users || []` so an undefined field can't reproduce the same class of crash
- **A white rounded panel appeared at the bottom of the screen while scrolling on a phone**, covering the bottom nav, on every page with a destination card. It was `.add-to-itinerary__sheet` — always mounted and hidden only by `transform: translateY(100%)`, which places it exactly one sheet-height below the viewport. When Chrome for Android collapses its toolbar during a scroll the viewport grows, and that just-off-screen sheet slides into view; it stayed visible for as long as a finger was held down, because that is when the toolbar is retracted. Now `visibility: hidden` when closed, with `visibility 0s linear 0.28s` on close so the slide-down still animates and `visibility 0s` on open so the slide-up is visible. `translateY(101%)` adds a pixel of slack against sub-pixel rounding
  - Diagnosis took several rounds because the symptom pointed elsewhere: the sheet's rounded top corners read as the phone's webview corners, and the first two theories (overscroll bounce exposing the canvas, then the bottom nav being displaced by the collapsing toolbar) both fit the screenshots. Colouring `html`, `body`, `#root` and `.bottom-nav` separately settled it — the box stayed white while everything around it changed, which is what identified it as an element at z-index 1000 rather than any background
- **Action rows overflowed their cards in French**, on desktop and on phones in landscape. Flex containers do not wrap unless told to, and flex items default to `min-width: auto`, so a row has no way to give when "Localisation" replaces "Location" or "Supprimer" replaces "Delete". Eight rows shared the pattern: `DestinationCard`, `DestinationManageCard`, `PendingRequestCard`, `CommentSection`, `ConfirmDialog`, `PreferencesModal`, `ReorderItineraryModal`, `RequestDetailModal`. All now wrap; the destination card's location button additionally gets `min-width: 0` with ellipsis so it truncates rather than pushing the favourite and add controls out of the card, while the comments button is `flex-shrink: 0` since a count cannot usefully truncate. The two card rows using `flex: 1` are now `flex: 1 1 120px` (and `100px` for the admin's three-button row) with `min-width: 0`, so buttons share space evenly and then wrap to a second line
- Reopening the app after closing it appeared to log the user out, even well inside the 30-day token lifetime. Nothing was actually clearing the token — `localStorage` survives a browser restart and only the Logout button calls `clearToken()`. The cause was that `/` always rendered `<Landing />` with no auth check, so returning users landed on the marketing page with its Log in and Sign up buttons and reasonably concluded their session had ended. `Landing.jsx` now redirects to `/home` when a token is present, with `replace: true` so the back button doesn't bounce a signed-in user onto it
- `api.js` now treats a 401 carrying a token as an expired session: it clears the token and user and redirects to `/login`. Guarded on `token` being present, so a 401 from an unauthenticated request (a wrong password on the login form) still surfaces its real error message, and skipped on public paths so a stale token plus Landing's public API calls cannot produce a redirect loop
- `index.css` — `html`/`body`/`#root` changed from `height: 100%` to `min-height: 100%`, and `overscroll-behavior-y: none` added. Not the cause of the white box above, but a hard `height` leaves those elements exactly one viewport tall while content overflows them, and disabling the rubber-band removes a whole class of "something appears below the page" reports
- `.destinations__filter-row` on Landing: the `@media (max-width: 480px)` block forced `grid-template-columns: 1fr` on `.landing__explore-grid`, overriding the `auto-fill` rule above it. The base rule already collapses to one column at that width, so the override only prevented narrower cards ever fitting two across. Its `minmax(280px, 1fr)` minimum is now wrapped in `min(280px, 100%)`, which stops the floor exceeding the container on a 320px device

### Changed
- `Destinations.jsx` / `Home.jsx` — filter rows are `<ScrollRow>` instead of `<div>`; the class name carries through to the inner track, so the existing flex and gap rules are untouched. Pills are `inline-flex` with a 6px gap for the icon, which is `flex-shrink: 0` so it survives the wrapping fixes above
- `App.jsx` — `Routes` moved into an `AppRoutes` component so it can call `useLocation` for the boundary's `resetKey`; `useLocation` cannot run in `App` itself, which is what renders the router. The boundary sits inside all three providers

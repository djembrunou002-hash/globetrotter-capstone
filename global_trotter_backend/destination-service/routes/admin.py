...
def _serialize_request(req, users_by_id, destinations_by_id):
    submitter = users_by_id.get(req["submitted_by"])

    display = None
    if req["type"] == "create":
        display = dict(req["payload"])
    else:
        destination = destinations_by_id.get(req["destination_id"])
        if destination:
    ...
    if display is not None and display.get("images"):
        display["images"] = absolute_images(display["images"])
...
def list_requests():
    ...
    users_by_id = fetch_users([r["submitted_by"] for r in requests])
    destinations_by_id = {d["id"]: d for d in load_json("destinations.json")["destinations"]}

    return jsonify({
        "requests": [_serialize_request(r, users_by_id, destinations_by_id) for r in requests]
    }), 200

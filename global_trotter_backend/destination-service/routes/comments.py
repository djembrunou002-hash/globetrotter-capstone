...
def _author(user_id, users_by_id):
...
def _serialize_node(node, children_by_parent, users_by_id, current_user_id=None):
    ...
    return {
        ...
        "author": _author(node["user_id"], users_by_id),
        ...
        "replies": [
            _serialize_node(child, children_by_parent, users_by_id, current_user_id) for child in children
        ],
    }


def _serialize_flat(node, users_by_id, current_user_id=None):
    likes = node.get("likes", [])
    dislikes = node.get("dislikes", [])
    return {
        "id": node["id"],
...
def get_comments(destination_id):
    ...
    users_by_id = fetch_users([node["user_id"] for node in all_nodes])
    ...
    return jsonify({
        "comments": [
            _serialize_node(root, children_by_parent, users_by_id, current_user_id) for root in roots
        ]
    }), 200
...
def add_comment(destination_id):
    ...
    users_by_id = fetch_users([user_id])

    return jsonify({"comment": _serialize_node(comment, {}, users_by_id, user_id)}), 201
...
def add_reply(destination_id, comment_id):
    ...
    return jsonify({"reply": _serialize_flat(reply, users_by_id, user_id)}), 201
...
def edit_comment(destination_id, comment_id):
    ...
    return jsonify({"comment": _serialize_flat(node, users_by_id, user_id)}), 200
